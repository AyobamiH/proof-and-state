...
| DoneState Marketplace development | Isolated and recovered; lifecycle proof active | OAuth App `3826463`; owner-only draft `donestate-marketplace-development`; signed ping and purchase recorded; recovery PR #52; development version `69e76740…`; production restoration version `fd8fe1b0…`; closure PR #53; exact cancellation and changed/pending-change deliveries remain open |
...
The owner-only development listing is a different external state from the submitted production listing. Its app, Worker, state, and credential target are isolated and recovered; the signed ping and purchase are recorded. The publisher reports cancelling that zero-cost test subscription, but the exact signed cancellation delivery and live changed and pending-change transitions remain unrecorded. Those gaps keep development lifecycle work active without changing production review state.
