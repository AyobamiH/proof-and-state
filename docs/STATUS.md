...
| DoneState Marketplace development lane | Isolated and active; lifecycle proof incomplete | OAuth App `3826463`; owner-only draft `donestate-marketplace-development`; development Worker version `be499906-19d4-4340-a968-e62aa5dc28d7`; signed ping `13cd1ca8-a4b8-11f1-888d-aba6875c1ba2`; signed purchase `90c3e110-a4b8-11f1-8357-8b375ae56683`; publisher-reported cancellation still lacks a recorded signed delivery ID |
...
The owner-only development listing is a different external state from the submitted production listing. Its separate app, secrets, Worker and state are operational, and one signed purchase reached the isolated webhook. The publisher reports cancelling that zero-cost test subscription, but the exact signed cancellation delivery and the live changed and pending-change transitions remain unrecorded. Those gaps keep development lifecycle work active and do not affect production review state.
