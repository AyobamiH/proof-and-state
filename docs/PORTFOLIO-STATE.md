...
| GTM-ORCH-001 — Prove the API-only GTM orchestrator | blocked | Proof & State maintainers | Replace CLOUDFLARE_API_TOKEN in the gtm-production GitHub environment with the secret value belonging to the account-owned token whose dashboard policy shows D1 Read and Edit. If that secret value is unavailable, roll or create the account-owned token and save its newly issued cfat_ value once. | Wait: Exact-head CI reports that GitHub is presenting a user-or-legacy credential, while the owner supplied policy evidence for a different account-owned token. Cloudflare resources, deployment and runtime health remain unproven. Re-entry: Resume by rerunning the environment-bound canary only after gtm-production holds the account-owned token value; do not alter the account ID or admin token. | 2026-09-15 |
