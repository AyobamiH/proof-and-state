...
| GTM-ORCH-001 — Prove the API-only GTM orchestrator | active | Proof & State maintainers | Run the environment-bound canary at the exact runtime-environment normalisation head and require migrations, publishing-disabled Worker deployment and exact-commit health read-back to pass before advancing the infrastructure gate. | Wait: The account-owned token, D1 and both Queues are provider-proven. Wrangler still needs the validated, whitespace-normalised account ID exported between CI steps; migrations, deployment and runtime health remain unproven. Re-entry: Resume from the first failed migration, deployment or health step; do not change credentials unless provider read-back proves a new credential-authority defect. | 2026-09-15 |
