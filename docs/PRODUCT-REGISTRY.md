...
| DoneState | [AyobamiH/donestate](https://github.com/AyobamiH/donestate) | Hosted 0.2.0 in OpenAI Review; production GitHub Marketplace OAuth listing submitted and under review; separate owner-only development draft active | Private maintenance App remains installed only on `AyobamiH/donestate`; production reports `Pending for publish`; development App `3826463` and draft `donestate-marketplace-development` are isolated, with signed ping and purchase recorded but cancellation delivery and live plan transitions still open; public operator decisions remain blocked | Bounded objectives, branches, PRs, durable event chain |
