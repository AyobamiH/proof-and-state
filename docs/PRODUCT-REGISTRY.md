...
| DoneState | [AyobamiH/donestate](https://github.com/AyobamiH/donestate) | Hosted 0.2.0 in OpenAI Review; GitHub Marketplace OAuth listing submitted and under review | Private maintenance App remains installed only on `AyobamiH/donestate`; the separate Marketplace OAuth flow, monotonic lifecycle webhook and incident process are live; GitHub reports `Pending for publish`; public operator aliases/address/regulatory/territory choices remain blocked | Bounded objectives, branches, PRs, durable event chain |
